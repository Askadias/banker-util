...
func (ea *EthereumAdapter) EstimateSendFee(ctx context.Context, w Wallet, coin string, amount float64, address string) (float64, error) {
	...
	if coin == "USDT" {
		...
		if err != nil {
			return 0, fmt.Errorf("unable to bind token contract: %v", err)
		}

		data, err := caller.PackTransferData(common.HexToAddress(address), etherToWei(amount, USDTDecimal))
		if err != nil {
...
func (ea *EthereumAdapter) EstimateMultiSendFee(ctx context.Context, w Wallet, coin string, amounts map[string]float64) (float64, error) {
	return 0, nil
}

func (ea *EthereumAdapter) MultiSend(ctx context.Context, w Wallet, coin string, amounts map[string]float64) (string, error) {
	return "", nil
}
